...
# Widget imports
from src.views.components.widgets.ledm_widget import LEDMWidget
from src.views.components.widgets.sirius_stream import SiriusStreamWidget
...
class SiriusScreenController(QObject):
    ...
    def _build_screen(self):
        ...
        self.ui_widget.set_password(self.config_manager.get("password", ""))
    ...
    def _wire_signals(self):
        ...
        if printer_ctrl:
            ...
            printer_ctrl.set_ip(self.ip if self.ip else "")
            printer_ctrl.set_credentials("admin", self.config_manager.get("password", ""))
            # Convert QPixmap to bytes for SiriusStreamWidget
            printer_ctrl.frame_ready.connect(self._on_frame_ready)
            printer_ctrl.connection_status.connect(self.ui_widget.set_status)
            ...
            # Update credentials when password changes
            self.ui_widget.password_changed.connect(
                lambda pwd: printer_ctrl.set_credentials("admin", pwd)
            )
        
        # === Printer Card Interactions ===
        self.printer_card.view_toggled.connect(self._toggle_sirius_connection)
        self.ui_widget.view_toggled.connect(self._toggle_sirius_connection)
        ...
        self.ui_widget.capture_requested.connect(self._on_capture_requested)
        self.ui_widget.password_changed.connect(lambda t: self.config_manager.set("password", t))
        self.ui_widget.password_changed.connect(self.manual_ops.set_password)
    ...
    def _update_password(self, pwd):
        ...
        self.ui_widget.set_password(pwd)
    ...
    def _on_frame_ready(self, pixmap):
        ...
        self.ui_widget.set_frame(image_bytes)
    
    # === UI Status ===
    
    def _on_connection_status(self, connected: bool, message: str):
        """Handle connection status from printer controller."""
        ...
        self.screen.status_message.emit(f"Sirius: {message}")
    
    def _on_ui_status_message(self, msg):
        """Handle status messages from stream widget."""
    ...
    def _toggle_sirius_connection(self):
        """Handle connection toggle from printer card or stream widget."""
        printer_ctrl = self._controllers.get('printer')
        if not printer_ctrl:
        ...
        if printer_ctrl.is_connected:
            ...
            printer_ctrl.start_stream()
    
    # === Capture Actions ===
    
    def _on_capture_requested(self, capture_type: str):
        """Handle capture requests from stream widget."""
        if capture_type == "UI":
            self._capture_ui("")
        else:
            self._capture_ecl(capture_type)
    
    def _capture_ui(self, description=""):
        """Capture UI screen."""
        printer_ctrl = self._controllers.get('printer')
        if printer_ctrl:
            printer_ctrl.capture_screen("UI")
        else:
            self.screen.error_occurred.emit("Printer controller not available")
    ...
    def _capture_ecl(self, description):
        ...
        printer_ctrl = self._controllers.get('printer')
        if printer_ctrl:
            printer_ctrl.capture_ecl(description)
        else:
            self.screen.error_occurred.emit("Printer controller not available")
    ...
    def update_ip(self, new_ip: str):
        ...
        self.ip = new_ip
        # Update printer controller with new IP
        printer_ctrl = self._controllers.get('printer')
        if printer_ctrl:
            printer_ctrl.set_ip(new_ip)
            # Update password if it changed
            password = self.config_manager.get("password", "")
            printer_ctrl.set_credentials("admin", password)
