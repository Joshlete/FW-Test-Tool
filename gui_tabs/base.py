...
class TabContent(ABC):
    ...
    def save_selected_alert(self, tree, alert_items):
        """
        Saves the selected alert to a file.
        ...
        try:
            ...
            # Use tkinter's filedialog to get a save location
            filename = tk.filedialog.asksaveasfilename(
                defaultextension=".json",
                filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
                initialfile=f"alert_{alert.get('id', 'unknown')}.json"
            )
            
            if not filename:  # User cancelled
                return
            
            # Save the alert to the file
            with open(filename, 'w') as f:
                json.dump(alert, f, indent=4)
            
            self._show_notification(f"Alert saved to {filename}", "green")
            
        except Exception as e:
            self._show_notification(f"Error saving alert: {str(e)}", "red")

    def view_alert_details(self, tree, alert_items):
        """
        ...
        try:
            ...
            # Create a new window to display the alert details
            details_window = tk.Toplevel(self.frame)
            details_window.title(f"Alert Details - {alert.get('id', 'Unknown')}")
            details_window.geometry("600x400")
            
            # Create a Text widget to show the JSON data
            text = tk.Text(details_window, wrap="none")
            y_scrollbar = ttk.Scrollbar(details_window, orient="vertical", command=text.yview)
            ...
            # Format the alert data as pretty JSON and insert it into the Text widget
            text.insert("1.0", json.dumps(alert, indent=4))
            ...
            self._show_notification(f"Error showing alert details: {str(e)}", "red")
